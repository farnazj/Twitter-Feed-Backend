...
async function advanceStage(userConditions, user) {
    ...
    let prevModelConfig = await user.getUserModelConfigs({
        where: {
            condition: mostRecentCondtion.stage
        }
    });

    let newConfig = await db.ModelConfig.create({
        version: 1,
        condition:  mostRecentCondtion.stage + 1,
        workspace: prevModelConfig.workspace
    });


    let allProms = [user.addUserCondition(newCondition), ...conditionProms, user.addUserModelConfigs(newConfig)];
