...
async function submitTrainingData(userId, tweetId, newLabelValue, whichModel) {
    ...
    console.log('going to set element label', workspace, tweetElementMapping.assignedId, valueText)
...
async function advanceStage(userConditions, user) {
    ...
    let newCondition = await db.Condition.create({
        version: 1,
        stage:  mostRecentCondtion.stage + 2, //TODO: change back to 1
...
async function getNextToLabel(userId, modelConfig, tweetsForStage) {
    ...
    logger.info(`elements to label next: ${elementsToLabel} for stage: ${modelConfig.forStage}`)
    ...
    logger.info(`this maps to tweet indices ${tweetsIndices}`)
    ...
    logger.info(`khob chi shod hala ${tweets.map(el => el.id)}`)
