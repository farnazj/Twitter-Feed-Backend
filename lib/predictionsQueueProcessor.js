...
function processRealTimeQueue(userIdSocketMapping) {
    ...
    predictionsQueue.on('completed', wrapAsync(async (job, result) => {
        ...
        if (result.modelChanged) {
            ...
            let tweetsWChangedLabels = allTweets.filter(tweet => !tweet.TweetAccuracyLabels.length ||
                (tweet.TweetAccuracyLabels.filter(label => label.version == 1)[0].AIAssigned == 1 && 
                (tweetIds.includes(tweet.id) && tweet.TweetAccuracyLabels.filter(label=> label.version == 1)[0].value == 0) || 
                    (!tweetIds.includes(tweet.id) && tweet.TweetAccuracyLabels.filter(label=> label.version == 1)[0].value == 1) ));

            console.log('positive tweets given by model', tweetIds),
            console.log('what I calculated', tweetsWChangedLabels.map(el => [el.id, el.TweetAccuracyLabels.filter(label=> label.version == 1)[0].value] ))
