...
function processRealTimeQueue(userIdSocketMapping) {
    ...
    predictionsQueue.process( async function (job, done) {
        ...
        try {
            ...
            logger.info(`models for user ${job.data.userId}, for stage ${job.data.modelConfig.forStage} : ${JSON.stringify(models)}`)
            ...
            logger.info( `model changed for user ${user.id}? ${modelChanged}, for stage: ${job.data.modelConfig.forStage}`);
    ...
    predictionsQueue.on('completed', wrapAsync(async (job, result) => {
        ...
        if (result.modelChanged && !result.errored) {  

            userCondition = (await user.getUserConditions({
                where: {
                  version: 1
                }
              }))[0];
        ...
        if (socketKey in userIdSocketMapping && !result.errored) {
            ...
            if (!connectivityChecks.includes(job.data.userId) ) {
                connectivityChecks.push(job.data.userId);

                setTimeout(async function() {
                    logger.info(`executing the timeout for user ${socketKey}, socket mapping keys are: ${Object.keys(userIdSocketMapping)}`);
                    if (!(socketKey in userIdSocketMapping)) {
                        logger.info(`removing the job ${JSON.stringify(job)} and workspace ${job.data.modelConfig.workspace} because user ${job.data.userId} is no longer connected to the ws`)
                        // await ;
                        await Promise.all([
                            sleuthServices.deleteWorkspace(job.data.modelConfig.workspace),
                            predictionsQueue.removeRepeatableByKey(job.opts.repeat.key)
                        ].map(p => p.catch(err => { logger.info(`could not delete the workspace ${job.data.modelConfig.workspace}  or remove the job  ${JSON.stringify(job)}: ${err}`)  })));
                    }

                    let userIndex = connectivityChecks.indexOf(job.data.userId);
                    connectivityChecks.splice(userIndex, 1);
